...
while True:
    ...
    if gaze.is_blinking():  # 눈을 감을 때
        if start_blinking == -9999: # 눈감고 있는 시간을 안재고 있었으면
            start_blinking = time()  # 눈감고 있는 시간 시작
            ...
            # 눈을 감았다 뜨면(깜빡이면)
            if (left_eye[0].y - left_eye[1].y) > 0.03 and (right_eye[0].y - right_eye[1].y) > 0.03:
                while True:
                    ...
                    # 눈을 감은지 10초가 초과하면
                    if start_blinking - time() > 5.0:
                        cv2.putText(frame, "Close your eyes for 10 seconds. Shut down the system", (10, 300), cv2.FONT_HERSHEY_DUPLEX, 0.9, (147, 58, 31), 2)
                        sleep(2)
                        #ScreenOFF()
                        start_blinking = -9999
                        break
